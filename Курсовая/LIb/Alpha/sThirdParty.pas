    ...
    if ImgList is TsAlphaImageList then
      ...
      if ImgList is TsCharImageList then begin
        ...
        if IsValidIndex(ImageIndex, cil.Count) then
          Result := cil.Items[ImageIndex].ActualWidth(False)//UseAngle)
        else
  ...
  if ImgList is TsAlphaImageList then
    ...
    if ImgList is TsCharImageList then begin
      ...
      if IsValidIndex(ImageIndex, cil.Count) then begin
        ...
        Result := Round(Result * cil.Items[ImageIndex].ScalingFactor);
//        if UseAngle and (cil.Items[ImageIndex].Angle <> 0) then
//          Result := Trunc(Abs(Result * acCos(cil.Items[ImageIndex].Angle)) + Abs(cil.SavedWidth * cil.Items[ImageIndex].ScalingFactor * acSin(cil.Items[ImageIndex].Angle))) + 1;
      end;
    end
    else
      if ImgList is TsVirtualImageList then
