...
{ TCustomNumEdit }
  ...
  private
    ...
    FFormatting: Boolean;
    FDisplayFormat: {$IFDEF RX_D3}String{$ELSE}PString{$ENDIF};
...
{$ENDIF}
    ...
    property DisplayFormat: string
      read {$IFDEF RX_D3}FDisplayFormat{$ELSE}GetDisplayFormat{$ENDIF}
...
begin
  ...
  FDisplayFormat := {$IFNDEF RX_D3}NewStr{$ENDIF}(DefaultDisplayFormat);
  FDecimalPlaces := 2;
  FZeroEmpty := True;
  inherited Text := '';
