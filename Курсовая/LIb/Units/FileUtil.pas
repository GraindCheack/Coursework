...
{$I RX.INC}
{$I-,R-}
{$IFDEF RX_HIDE_PLATFORM_WARNINGS}
...
uses {$IFDEF WIN32} {$IFDEF RX_D3} ActiveX, ComObj, ShlObj, {$ELSE} Ole2,
  ...
  RxPrgrss {$IFDEF RX_D6}, RTLConsts{$ENDIF};
...
begin
  ...
  FObjectInstance := {$IFDEF RX_D6}Classes.{$ENDIF}MakeObjectInstance(WndProc);
  FDesktopRoot := True;
...
begin
  if FObjectInstance <> nil then {$IFDEF RX_D6}Classes.{$ENDIF}FreeObjectInstance(FObjectInstance);
  inherited Destroy;
end;
...
{$IFDEF RX_D6}
        if not SHGetPathFromIDList(ItemIDList, Temp) then RaiseLastOSError;
{$ELSE}
  ...
  try
    ...
    try
      ...
      if OverwriteReadOnly then begin
        Attr := {$IFDEF RX_D6}GetFileAttributes(PChar(Destination)){$ELSE}FileGetAttr(Destination){$ENDIF};
        if (Attr >= 0) and ((Attr and faReadOnly) <> 0) then
{$IFDEF RX_D6}
          SetFileAttributes(PChar(Destination), Attr and not faReadOnly)
{$ELSE}
...
begin
  ...
  if not RenameFile(FileName, Destination) then begin
    Attr := {$IFDEF RX_D6}GetFileAttributes(PChar(FileName)){$ELSE}FileGetAttr(FileName){$ENDIF};
    if Attr < 0 then Exit;
    if (Attr and faReadOnly) <> 0 then
{$IFDEF RX_D6}
      SetFileAttributes(PChar(FileName), Attr and not faReadOnly);
{$ELSE}
...
var
  FileAttr: Integer;
begin
  FileAttr := {$IFDEF RX_D6}GetFileAttributes(PChar(FileName)){$ELSE}FileGetAttr(FileName){$ENDIF};
...
{$IFDEF RX_D6}
            SetFileAttributes(PChar(NormalDir(Path) + FileInfo.Name), faArchive);
{$ELSE}
