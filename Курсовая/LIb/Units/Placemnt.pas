...
{ TFormPlacement }
  ...
  private
    ...
    FIniSection: {$IFDEF RX_D3}String{$ELSE}PString{$ENDIF};
    FIniFile: TIniFile;
...
begin
  Result := FIniSection{$IFNDEF RX_D3}^{$ENDIF};
...
begin
  if Assigned(FStorage) then Result := FStorage.FIniSection{$IFNDEF RX_D3}^{$ENDIF}
  else Result := '';
