...
var
  FieldTypes: array[TFieldType] of {$IFDEF RX_D3}String{$ELSE}PString{$ENDIF};
...
{$ENDIF}
      TypeList.Items.Add(FieldTypes[J]{$IFNDEF RX_D3}^{$ENDIF});
...
{$ENDIF}
      begin
        with TypeList do ItemIndex := Items.IndexOf(FieldTypes[DataType]{$IFNDEF RX_D3}^{$ENDIF});
