...
{ TDBStatusLabel }
  ...
  private
    FDataLink: TDataLink;
    FDataSetName: {$IFDEF RX_D3}String{$ELSE}PString{$ENDIF};
...
uses SysUtils, RxStrUtils, Dialogs, ExtCtrls, DbConsts, RxAppUtils, VCLUtils,
  DbUtils, {$IFNDEF RX_D3} BdeUtils, {$ENDIF} PickDate, RxCalc, MaxMin,
  RxDConst {$IFDEF RX_D6}, Variants{$ENDIF};
...
begin
//miki
//mik  for I := Low(TGridPicture) to High(TGridPicture) do GridBitmaps[I].Free;
  for I := Low(TGridPicture) to High(TGridPicture) do
  Begin
     if GridBitmaps[I] <> nil then
        GridBitmaps[I].Free;
     GridBitmaps[I]:=nil;
  end;
//mikf
...
begin
  Result := FDataSetName{$IFNDEF RX_D3}^{$ENDIF};
  if not (csDesigning in ComponentState) then begin
