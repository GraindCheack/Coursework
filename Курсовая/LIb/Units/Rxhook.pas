...
begin
  inherited Create;
  FList := TList.Create;
  FNewWndProc := {$IFDEF RX_D6}Classes.{$ENDIF}MakeObjectInstance(HookWndProc);
  FPrevWndProc := nil;
  FControl := nil;
end;
...
begin
  ...
  FControl := nil;
  FList.Free;
  {$IFDEF RX_D6}Classes.{$ENDIF}FreeObjectInstance(FNewWndProc);
  FNewWndProc := nil;
  inherited Destroy;
end;
...
begin
  inherited Create;
  FHandle := {$IFDEF RX_D6}Classes.{$ENDIF}AllocateHWnd(WndProc);
end;

destructor THookList.Destroy;
begin
  while Count > 0 do TControlHook(Last).Free;
  {$IFDEF RX_D6}Classes.{$ENDIF}DeallocateHWnd(FHandle);
  inherited Destroy;
end;
