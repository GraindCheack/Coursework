...
{$I RX.INC}
{$IFDEF RX_HIDE_PLATFORM_WARNINGS}{$WARN UNIT_PLATFORM OFF}{$ENDIF} // for FileCtrl
...
type
  ...
  private
    FTitle: {$IFDEF RX_D3}String{$ELSE}PString{$ENDIF};
...
{$ELSE}
      read GetDialogTitle write SetDialogTitle
{$ENDIF}
      stored IsCustomTitle;
...
uses
  {$IFDEF RX_D6}RTLConsts, Variants{$ELSE}Consts{$ENDIF},
  ShellAPI, {$IFDEF RX_D3} ExtDlgs, {$ENDIF} RXCConst, VCLUtils,
...
begin
  inherited Create(AOwner);
  FBlanksChar := ' ';
  FTitle := {$IFNDEF RX_D3}NewStr{$ENDIF}(LoadStr(SDateDlgTitle));
...
begin
  Result := (CompareStr(LoadStr(SDateDlgTitle), DialogTitle) <> 0) and
    (FTitle <> {$IFDEF RX_D3}''{$ELSE}NullStr{$ENDIF});
end;
