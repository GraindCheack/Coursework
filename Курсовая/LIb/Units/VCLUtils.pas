...
Uses SysUtils, Messages, MaxMin, Consts, RxConst, {$IFDEF RX_V110} SysConst, {$ENDIF}
  ...
  {$IFDEF RX_D6}, RTLConsts, Variants{$ENDIF};
...
{$IFDEF RX_HIDE_PLATFORM_WARNINGS}{$WARN SYMBOL_PLATFORM OFF}{$ENDIF}
function AllocMemo(Size: Longint): Pointer;
begin
  if Size > 0 then
...
{$IFDEF RX_HIDE_PLATFORM_WARNINGS}{$WARN SYMBOL_PLATFORM ON}{$ENDIF}
...
{$IFDEF RX_D6}
    if GetTempPath(MAX_PATH, Path) = 0 then RaiseLastOSError;
    if GetTempFileName(Path, 'ANI', 0, FileName) = 0 then RaiseLastOSError;
{$ELSE}
...
{$IFDEF RX_D3}
var
  Error: {$IFDEF RX_D6}EOSError{$ELSE}EWin32Error{$ENDIF};
{$ENDIF}
begin
  if ErrorCode <> ERROR_SUCCESS then begin
{$IFDEF RX_D3}
    Error := {$IFDEF RX_D6}EOSError{$ELSE}EWin32Error{$ENDIF}.CreateFmt(
      {$IFDEF RX_D6}SOSError{$ELSE}SWin32Error{$ENDIF}, [ErrorCode,
