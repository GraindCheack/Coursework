...
{$IFDEF WIN32}
    ...
    function GetEditStyle(ACol, ARow: Longint):
      {$IFDEF RX_D6}TEditStyle; override;{$ELSE}TInplaceEditStyle; dynamic;{$ENDIF}
...
uses SysUtils, VCLUtils, MaxMin, Consts, RxAppUtils;
...
function TRxDrawGrid.GetEditStyle(ACol, ARow: Longint):
  {$IFDEF RX_D6}TEditStyle{$ELSE}TInplaceEditStyle{$ENDIF};
begin
  Result := ieSimple;
  if Assigned(FOnGetEditStyle) then FOnGetEditStyle(Self, ACol, ARow,
    {$IFDEF RX_D6}TInplaceEditStyle(Result){$ELSE}Result{$ENDIF});
end;
